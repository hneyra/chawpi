...
class ObjectCrudApiTest : FullAppIntegrationTest() {
    @Autowired
    ...
    @Test
    fun `a field an automation depends on cannot be deleted, and says which rule`() {
        client
            .post()
            .uri("/api/metadata/objects/$objectName/fields")
            .header(HttpHeaders.AUTHORIZATION, token)
            .bodyValue(mapOf("name" to "revisado", "type" to "BOOLEAN"))
            .exchange()
            .expectStatus()
            .isCreated

        val rule = uniqueName("marca")
        client
            .post()
            ...
            .bodyValue(
                mapOf(
                    "name" to rule,
                    "label" to "Marca revisado",
                    "definition" to
                        mapOf(
                            "trigger" to mapOf("type" to "RECORD_CREATED"),
                            "actions" to listOf(mapOf("type" to "UPDATE_FIELD", "field" to "revisado", "value" to "true"))
                        )
                )
            ).exchange()
            .expectStatus()
        ...
        client
            .delete()
            .uri("/api/metadata/objects/$objectName/fields/revisado")
            .header(HttpHeaders.AUTHORIZATION, token)
            .exchange()
            .expectStatus()
            .isEqualTo(409)
            .expectBody()
            .jsonPath("$.detail")
            .value<String> { assertThat(it).contains(rule) }

        assertThat(columnExists("revisado")).isTrue()
    }
