...
class PermissionEnforcementTest : FullAppIntegrationTest() {
                ...
                mapOf(
                    ...
                    "fields" to
                        listOf(
                            ...
                            mapOf("name" to "zona", "type" to "GEOMETRY", "geometryType" to "POINT", "srid" to 4326)
                        )
                )
            ).exchange()
            .expectStatus()
    ...
    // a geometry is a field, so its permissions are the field's. checking only attributes would
    // have let a locked geometry in through the other door.
    @Test
    fun `a locked geometry can be neither written nor read back`() {
        val role = newRole("Blind")
        grant(role, listOf("READ", "CREATE", "UPDATE"))
        restrictField(role, "zona", read = false, write = false)
        val token = newUserToken(role)
        val point = mapOf("type" to "Point", "coordinates" to listOf(-77.0, -12.0))

        client
            .post()
            .uri("/api/objects/$objectName/records")
            .header(HttpHeaders.AUTHORIZATION, token)
            .bodyValue(mapOf("attributes" to mapOf("codigo" to "A-1"), "geometries" to mapOf("zona" to point)))
            .exchange()
            .expectStatus()
            .isBadRequest
            .expectBody()
        ...
        // one the admin drew never reaches them either
        client
            .post()
            .uri("/api/objects/$objectName/records")
            .header(HttpHeaders.AUTHORIZATION, admin)
            .bodyValue(mapOf("attributes" to mapOf("codigo" to "A-2"), "geometries" to mapOf("zona" to point)))
            .exchange()
            .expectStatus()
            .isCreated
        ...
        client
            ...
            .isOk
            .expectBody()
            .jsonPath("$.content[0].geometries.zona")
            .doesNotExist()
    }
